import sequtils, strutils, tables, itertools, sugar, algorithm, times, sets

let pp = toSeq(lines("d12c.in")).mapIt(it.split("-")).mapIt(@[(it[0], it[1]), (it[1], it[0])]).foldl(a&b)
    ...
    if a[i].len > 0 and a[i][^1] != "end":
      ...
      for k, v in s:
        if k[0] in 'a'..'z' and v == 2:
          c = true
      for xx in p[a[i][^1]]:
        if xx[0] in 'a'..'z' and c and xx in s:
          continue
        a.add a[i]&xx
        result = true
      a[i].setLen 0
